...
class Analyzer {
  ...
  getCallChains() {
    ...
    for (const [sessionId, sessionCalls] of sessions.entries()) {
      sessionCalls.sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp));
    }
    ...
    sessions.forEach((sessionCalls, sessionId) => {
      ...
      const chain = {
        ...
        totalDuration: sessionCalls.reduce((sum, call) => sum + (call.duration || 0), 0)
      };
  ...
  getPerformanceAnalysis() {
    const calls = this.db.getAllCalls().filter(call => call.status === 'success' && call.duration != null);
    ...
    tools.forEach((durations, tool) => {
      ...
      const variance = durations.reduce((sum, duration) =>
        sum + Math.pow(duration - avg, 2), 0) / durations.length;
    ...
    // 获取最慢和最快的工具（基于平均时间）
    const sortedByAvg = Object.entries(performanceByTool)
    ...
    const slowest = sortedByAvg.slice(-5).reverse().map(([tool, data]) => ({
      tool,
      avg: data.avg,
      count: data.count
    }));
    ...
    // 获取最稳定的工具（标准偏差最小）
    const sortedByStdDev = Object.entries(performanceByTool)
   ...
   */
  getErrorAnalysis() {
    const calls = this.db.getAllCalls().filter(call => call.status === 'error');
    ...
    calls.forEach(call => {
      errorsByServer.set(call.server, (errorsByServer.get(call.server) || 0) + 1);
    });
