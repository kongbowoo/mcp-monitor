class MCPMonitor {
    ...
    async loadStats() {
        try {
            ...
            if (data.success) {
                ...
                document.getElementById('successfulCalls').textContent = data.data.successfulCalls;
                document.getElementById('failedCalls').textContent = data.data.failedCalls;
                document.getElementById('avgDuration').textContent = data.data.avgDuration;
                document.getElementById('toolCount').textContent = Object.keys(data.data.tools).length;
                document.getElementById('serverCount').textContent = Object.keys(data.data.servers).length;
    ...
    async loadCalls() {
        try {
            ...
            const url = status ? `${this.baseUrl}/calls?status=${status}` : `${this.baseUrl}/calls`;
    ...
    renderCallGraph(graphData) {
        ...
        const node = g.append('g')
            ...
            .enter().append('circle')
            .attr('r', d => d.type === 'server' ? 25 : 15)
            .attr('fill', d => d.type === 'server' ? '#667eea' : '#f093fb')
            .call(d3.drag()
